...
while read -r URL || [ -n "$URL" ]; do
    ...
    # ZMIANA: --update-to nightly oraz dodatkowe parametry extractora
    yt-dlp --update-to nightly --cookies "$COOKIES_FILE" --user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0 Safari/537.36" --extractor-args "youtube:player-client=android,ios;skip=webpage_signature" --force-ipv4 --match-filter "live_status != upcoming" -f "ba/b" --extract-audio --audio-format mp3 --audio-quality 0 --playlist-end 2 --no-warnings --ignore-errors --no-mtime --download-archive "$DATA_DIR/downloaded.txt" --output "$DATA_DIR/%(upload_date)s-%(title)s.%(ext)s" "$URL"
